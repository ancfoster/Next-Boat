...
def CreateDeleteFavourite(request, id):
    ...
    existing_favourite = Favourites.objects.filter(Q(favourite_created_by=request.user) & Q(listing=listing_object.pk))  # noqa
...
class ShowFavourites(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        return Favourites.objects.filter(favourite_created_by=self.request.user)  # noqa
